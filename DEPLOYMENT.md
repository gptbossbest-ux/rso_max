...
Новые и переименованные логины ограничены 128 символами. Существующие legacy-
логины большей длины продолжают поддерживаться при точном вводе (с учётом
регистра). Перед обновлением их можно найти запросом
`SELECT id, username FROM users WHERE length(username) > 128;` и безопасно
переименовать в разделе «Пользователи» веб-панели.
